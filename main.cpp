...
int mainasdawd(int argc, char **argv) {
    ConvexPolygon cp1, cp2;

    /*
        cp1.push_vertex(Vector3d(-1.0, 11.0, 0.0));
        cp1.push_vertex(Vector3d(-1.0, 5.0, 0.0));
        cp1.push_vertex(Vector3d(4.0, 9.0, 0.0));
    */
    ...
    cp2.push_vertex(Vector3d(5.0,  7.0, 0.0));
    cp2.push_vertex(Vector3d(7.0,  3.0, 0.0));
    cp2.push_vertex(Vector3d(10.0, 2.0, 0.0));
    cp2.push_vertex(Vector3d(12.0, 7.0, 0.0));

    Vector3d movement = Vector3d(1.0, 0.0, 0.0),
                 pace = Vector3d(1.0, 0.0, 0.0);
    while (1) {
		ConvexPolygon a, b;
        RigidBody r1, r2;

		for (size_t i = 0; i < 3; ++i) {
            a.push_vertex(cp1.getVertices()[i] + movement);
		}
		for (size_t i = 0; i < 4; ++i) {
            b.push_vertex(cp2.getVertices()[i]);
		}

		r1.push_polygon(a);
        r2.push_polygon(b);
		if (r1.collidesWith(r2.getPolygons(), r2.getPosition()))
		{
			printf("Collided at:\n\t{%lf, %lf}, {%lf, %lf}, {%lf, %lf}\n\t{%lf, %lf}, {%lf, %lf}, {%lf, %lf}, {%lf, %lf}\n\n",
				a.getVertices()[0][X], a.getVertices()[0][Y], a.getVertices()[1][X], a.getVertices()[1][Y], a.getVertices()[2][X], a.getVertices()[2][Y],
				b.getVertices()[0][X], b.getVertices()[0][Y], b.getVertices()[1][X], b.getVertices()[1][Y], b.getVertices()[2][X], b.getVertices()[2][Y], b.getVertices()[3][X], b.getVertices()[3][Y]
			);
			exit(1);
		}
		else
